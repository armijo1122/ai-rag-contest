...
async def generation_with_knowledge_llm_retrieval(
    ...
    # use llm query to retrieve
    node_with_scores = await retriever.aretrieve(query_bundle_llm)

    if debug:
        print(f"retrieved:\n{node_with_scores}\n------")
    if reranker:
        # rerank retrieved data
        node_with_scores = reranker.postprocess_nodes(node_with_scores, query_bundle_llm)
        if debug:
            print(f"reranked:\n{node_with_scores}\n------")
    # combine data after rerank
    context_str = "\n\n".join(
        [f"{node.metadata['document_title']}: {node.text}" for node in node_with_scores]
    )
